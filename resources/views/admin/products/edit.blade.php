            ...
            <div class="bg-white p-6 shadow sm:rounded-lg">
                ...
                <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    @method('PUT') <div class="mb-4">
                        ...
                        <x-text-input id="name" name="name" type="text" class="block mt-1 w-full" value="{{ $product->name }}" required />
                    </div>

                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            <x-input-label for="price" value="Price ($)" />
                            <x-text-input id="price" name="price" type="number" step="0.01" class="block mt-1 w-full" value="{{ $product->price }}" required />
                        </div>
                        <div>
                            <x-input-label for="quantity" value="Stock Quantity" />
                            <x-text-input id="quantity" name="quantity" type="number" class="block mt-1 w-full" value="{{ $product->quantity }}" required />
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                  <x-input-label value="Current Product Image" />
                   @if($product->image)
                  <div class="mt-2 mb-4">
                  <img src="{{ asset('storage/' . $product->image) }}" class="w-32 h-32 object-cover rounded border">
                   ...
                   @endif
                    ...
                    <x-input-label for="image" value="Upload New Image (Optional)" />
